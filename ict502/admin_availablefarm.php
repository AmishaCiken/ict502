...
// Update the farm query to include STATUS
$query = "
    SELECT f.FARMID, f.FARMNAME, f.FARMLOCATION, f.FARMSIZE, f.WATERSOURCE, f.STATUS,
...
// Add this with other POST handlers
// Handle Farm Update
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_farm'])) {
    ...
    if (oci_execute($stmt)) {
        $_SESSION['message'] = "Farm updated successfully!";
    } else {
        $error = oci_error($stmt);
        $_SESSION['message'] = "Error updating farm: " . $error['message'];
    }
    header("Location: admin_availablefarm.php");
    exit();
}
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button class="btn btn-warning btn-sm me-2" 
                                ...
                                data-farmlocation="<?= $farm['FARMLOCATION'] ?>"
                                data-farmsize="<?= $farm['FARMSIZE'] ?>"
                                data-watersource="<?= $farm['WATERSOURCE'] ?>"
...
<!-- Add this after the Add Farm Modal -->
<!-- Edit Farm Modal -->
            ...
            <div class="modal-body">
                <form method="POST" action="admin_availablefarm.php">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="farmname" id="editFarmName" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Farm Location</label>
                        <input type="text" class="form-control" name="farmlocation" id="editFarmLocation" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Farm Size (ha)</label>
                        <input type="number" class="form-control" name="farmsize" id="editFarmSize" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Water Source</label>
                        <input type="text" class="form-control" name="watersource" id="editWaterSource" required>
                    </div>
                    <div class="mb-3">
                    ...
                    <button type="submit" name="update_farm" class="btn btn-primary">Save Changes</button>
                </form>
            </div>
        </div>
    </div>
...
// Edit Farm Modal Handler
document.getElementById('editFarmModal').addEventListener('show.bs.modal', function(event) {
    const button = event.relatedTarget;
    document.getElementById('editFarmId').value = button.dataset.farmid;
    document.getElementById('editFarmName').value = button.dataset.farmname;
    document.getElementById('editFarmLocation').value = button.dataset.farmlocation;
    document.getElementById('editFarmSize').value = button.dataset.farmsize;
    document.getElementById('editWaterSource').value = button.dataset.watersource;
